...

{
  ...
  boot.loader.systemd-boot.extraInstallCommands =
    ...
    in
    ''
      for conf in ${config.boot.loader.efi.efiSysMountPoint}/loader/entries/nixos-generation-*.conf; do
        gen=$(${pkgs.coreutils}/bin/basename "$conf" | ${pkgs.gnused}/bin/sed -n 's/^nixos-generation-\([0-9]\+\).*/\1/p')
